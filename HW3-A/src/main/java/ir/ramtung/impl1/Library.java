...
public class Library implements ir.ramtung.sts01.ILibrary {
    ...
    public int getLoanDueDate(String memberName, String documentTitle) throws InvalidArgumentEx {
        Loan loan = findLoan(memberName, documentTitle);
        if (loan == null) {
            throw new InvalidArgumentEx("Loan not found for the member and document specified");
        }
        return loan.dueDate;
    }

    // Add this method to get the loan duration of a document
    public int getDocumentLoanDuration(String documentTitle) throws InvalidArgumentEx {
        Document document = findDocument(documentTitle);
        if (document == null) {
            throw new InvalidArgumentEx("Document not found");
        }
        return document.loanDuration();
    }
    @Override
